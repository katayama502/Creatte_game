...
const App = () => {
  ...
  async function handleRoundEnd(finalPlayers) {
    ...
    // Host logic normally, but allow any player to force update if stuck (checking if round matches local)
    // We strictly check onlineRole === 1 for auto-update, but if invoked manually (e.g. from button), we might want to bypass.
    // For simplicity, let's allow ANYONE to send the "next round" signal if they calculate it.
    // But to avoid double triggers in normal flow, we keep the check for normal flow.
    // The "Force Button" will call this. We need to distinguish or just rely on server handling race (last write wins).
    // Let's rely on "Last Write Wins" for failsafe.

    // Modifying condition: If P1 (Host) OR it is an emergency manual trigger...
    // Actually, simply allowing both to send is fine if they calculate same result.
    // But generating hands might differ! Random seed!
    // If P2 generates hands, they will differ from P1.
    // This is fine for emergency fix. 

    // To be safe: Only P1 auto-sends.
    // If Force Button is clicked, we bypass.
    // How to know if force button? 
    // Let's just allow it for now. The server deep merge fixes the program loss.
    // The main issue is "Hanging".
    // I will enable sending for everyone, but maybe add a small random delay or relying on user click for P2.
    // Since `handleRoundEnd` is called automatically by `runExecutionLocal`, if I enable it for P2, it will ALWAYS send.
    // That causes race condition on Hand Generation.

    // Safe Fix:
    // Only P1 sends automatically.
    // The Force Button calls `forceRoundEnd`.

    // Let's implement `forceRoundEnd` separatly or modify `handleRoundEnd` to accept a flag.
  } else if (onlineRole === 1 || window.forceNext) {
    // ... existing logic ...
  }
  // No, creating a separate function is cleaner but I need to reuse logic.
  // Let's assume `handleRoundEnd` is auto-called.
...
async function forceNextRound() {
  ...
  const p2W = players[1]?.x === 0 && players[1]?.y === 0;
  let localWinner = p1W && p2W ? 'DRAW' : p1W ? 1 : p2W ? 2 : null;

  // Generate hands (even if P2, we force it)
  ...
  socket.emit('update_state', {
    roomId, updates: {
      players: players.map(({ customImage, ...rest }) => rest),
      round: round + 1,
      gameState: 'PLANNING',
      activePlayer: 1,
      hands: serializedHands,
      programs: { 1: Array(5).fill(null), 2: Array(5).fill(null) },
      message: `ラウンド ${round + 1}：${players[0].name} のターン`
    }
  });
}
  }
      ...
      <div className="lg:col-span-6 flex flex-col items-center">
        ...
        {gameState === 'EXECUTION' && (
          <button
            ...
            className="mb-6 px-6 py-2 bg-slate-200 text-slate-500 text-xs font-black rounded-full hover:bg-slate-300 transition-all shadow-sm"
          >
            進行が止まった場合はここを押してください
          </button>
        )}
